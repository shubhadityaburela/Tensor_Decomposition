...
from data_generation import generate_data_crossing_wave, generate_data_crossing_wave_sine
from minimizer_helper import generate_phi
import os

# Plots the results
impath = "plots/Sine_StraightCubic_wave/"  # For plots
immpath = "data/Sine_StraightCubic_wave/"  # For data
...
class Parameters:
    ...
    n: int = 400
    m: int = 200
    K: np.ndarray = np.asarray([1, 1])  # We use a single type of basis in each frame
    type_basis: np.ndarray = np.asarray([["gaussian"], ["gaussian"]])  # We use a single Gaussian basis for each frame
    K_st: np.ndarray = np.asarray([0, 1, 2])  # Just to get the indexing access of the array right
    sigma: np.ndarray = np.asarray([[4.0], [4.0]])  # Gaussian variance for each frame
    ...
    degree: np.ndarray = np.asarray([2, 4])  # We use a linear polynomial and a cubic polynomial for both the frames respectively
    degree_st: np.ndarray = np.asarray([0, 2, 6])  # Just to get the indexing access of the array right
    beta_init = [[-5.0, -3.1], [-0.01, -0.0, -0.7, 1.0]]  # Initial guess value for the coefficients of the shifts
    type_shift = ["polynomial", "polynomial"]  # We use polynomial shifts for both the frames
    beta = [[-18.0, 2.0], [0.15, 0.0, 0.8, 1.5]]
    center_of_matrix: np.ndarray = np.asarray([200.0, 200.0])

    alpha_init: np.ndarray = np.ones((sum(K), m))
    alpha_solver_ck: float = 1e6
    alpha_solver_lamda_1: float = 1e-3

    beta_solver_dk: float = 1.0
    beta_solver_tau: np.ndarray = np.asarray([3e-5, 3e-5, 5e-6, 5e-6, 5e-6, 5e-6])
...
if __name__ == '__main__':
    ...
    X, T = np.meshgrid(param.x, param.t)

    # Generate the data
    Q, _, _ = generate_data_crossing_wave_sine(param)
    ...
    # vmin = np.min(Q)
    # vmax = np.max(Q)
    # im1 = ax1.pcolormesh(X.T, T.T, Q, vmin=vmin, vmax=vmax, cmap=cmap)
    # ax1.set_xlabel(r"$x$")
    # ax1.set_ylabel(r"$t$")
    # ax1.axis('off')
