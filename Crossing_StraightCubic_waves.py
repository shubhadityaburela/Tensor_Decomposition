...
from data_generation import generate_data_crossing_wave
from minimizer_helper import generate_phi
import os

# Plots the results
impath = "plots/StraightCubic_wave/"  # For plots
immpath = "data/StraightCubic_wave/"  # For data
...
class Parameters:
    ...
    degree: np.ndarray = np.asarray([2, 4])  # We use a linear and a cubic polynomial for both the frames respectively
    ...
    beta_solver_tau: np.ndarray = np.asarray([1e-4, 1e-4, 1e-8, 1e-8, 1e-5, 1e-5])
...
if __name__ == '__main__':
    ...
    # Generate the data
    Q, _, _ = generate_data_crossing_wave(param)
