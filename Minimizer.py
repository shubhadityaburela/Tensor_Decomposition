...
def gradient_H_alpha(Q, alpha, beta, param):
    ...
    dH_dAlpha = - 2 * oe.contract('ij,kij->kj', R, phiBeta)
...
def gradient_H_beta(Q, alpha, beta, param):
    ...
    for nf in range(param.nf):
        dH_dBeta[param.degree_st[nf]:param.degree_st[nf + 1]] = -2 * oe.contract('ij,ijkl,kj->l', R, phiBetaJac[nf],
                                                                                 alpha[param.K_st[nf]:param.K_st[nf + 1], :])
        # print(np.linalg.norm(dH_dBeta[param.degree_st[nf]:param.degree_st[nf + 1]]))
    return dH_dBeta
...
def argmin_H_beta(Q, alpha, beta, param):
    ...
    for n in range(param.beta_solver_maxit):
        ...
        # 1) Primal update
        beta_half = beta - (param.beta_solver_tau / param.beta_solver_dk) * gradient_H_beta(Q, alpha, beta,
                                                                                            param) - param.beta_solver_tau * u
        # beta_half[beta_half < 0] = 0  # Proximal operator is equal to the projection for indicator function

        # 2) Dual update
        uTemp = u / param.beta_solver_sigma + (2 * beta_half - beta)
        u_half = uTemp - prox_l1(uTemp, (param.beta_solver_lamda_2 / (param.beta_solver_dk * param.beta_solver_sigma)))

        # 3) Inertial update
        beta = beta + rho * (beta_half - beta)
        u = u + rho * (u_half - u)

        # 4) Check stopping criterion (convergence in terms of objective function)
        crit_old = crit
        crit = J(Q, alpha, beta, param)
        if np.abs(crit_old - crit) < param.beta_solver_gtol * np.abs(crit):
...
def argmin_H(Q, param):
    ...
    alpha = param.alpha_init     # np.random.rand(sum(param.K), param.m)
    ...
    for it in range(param.maxit):
        ...
        if abs(obj_val - old_obj) < param.gtol * np.abs(old_obj):
            print(f"Outer loop converged after {it} iterations")
            if rec_err > 0.01:
                param.beta_solver_tau = param.beta_solver_tau / 1e3
                param.alpha_solver_ck = param.alpha_solver_ck / 1e3
            else:
                break
